...
- Use Clerk Organization plans and an organization pricing table. Do not keep user-level subscriptions for the new offers.
...
- Convex stores the Clerk Organization identifier, Clerk subscription identifiers, plan slug, billing period, subscription status, trial dates, confirmed Editor quantity, Storage Add-on quantity, last Clerk event time, and reconciliation state for each Workspace.
