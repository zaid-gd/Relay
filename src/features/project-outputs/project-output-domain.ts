...
export function normalizeMediaUrl(input: string): MediaUrlResult {
  ...
  if (YOUTUBE_HOSTS.has(host)) {
    if (!youtubeId)
      return {
        ok: false,
        error: "That YouTube URL does not contain a valid video.",
      };
    return {
      ok: true,
      value: {
  ...
  if (VIMEO_HOSTS.has(host)) {
    if (!vimeoId)
      return {
        ok: false,
        error: "That Vimeo URL does not contain a valid video.",
      };
    return {
      ok: true,
      value: {
...
export function addMediaVersion(
  ...
  const outputVersions = versions.filter(
  ...
  const version: MediaVersion = {
    ...
    versionNumber:
      Math.max(0, ...outputVersions.map((item) => item.versionNumber)) + 1,
    source: input.source,
...
export function selectCurrentMediaVersion(
  ...
  const selected = output.currentVersionId
    ? versions.find((version) => version.id === output.currentVersionId)
    : undefined;
...
export function unresolvedOldVersionComments(
  ...
  return comments.filter(
    (comment) =>
      !comment.resolved && comment.mediaVersionId !== currentVersionId
  );
}
