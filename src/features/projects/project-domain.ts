...
export function getProjectWorkflowStage(project: Pick<WorkItem, "workflowStageId" | "workflowStage" | "workflowStages" | "status">): WorkflowStage {
  ...
  const current = currentStage ? stages.find((stage) => stageMatches(stage, currentStage)) : undefined;
  if (current) return current;
  if (project.status === "Delivered") return stages.find((stage) => stage.purpose === "delivered") ?? stages.at(-1) ?? DEFAULT_WORKFLOW_STAGES.at(-1)!;
  if (project.status === "Planned") return stages.find((stage) => stage.purpose === "planned") ?? stages[0] ?? DEFAULT_WORKFLOW_STAGES[0];
  const exact = stages.find((stage) => stage.label === project.status);
  if (exact) return exact;
  const matchingPurpose = stages.find((stage) => getWorkflowStageStatus(project, stage) === project.status);
...
export function getWorkflowStageStatus(
  ...
  if (resolved) {
    ...
    if (resolved.purpose === "client_review" || resolved.purpose === "approved") return "Review";
    return "In Progress";
  }
  const normalized = typeof stage === "string" ? stage.trim().toLowerCase() : stage.label.trim().toLowerCase();
  if (normalized === "cancelled" || normalized === "canceled") return "Cancelled";
  if (normalized.includes("deliver") || normalized.includes("publish") || normalized.includes("export")) return "Delivered";
  if (normalized.includes("revision")) return "Revision";
  if (normalized.includes("review") || normalized.includes("approv")) return "Review";
  return "In Progress";
}
...
export function resolveProjectWorkflowStage(
  ...
  const requestedStatus = getWorkflowStageStatus({ workflowStages: DEFAULT_WORKFLOW_STAGES }, requestedStage);
  return stages.find((stage) => getWorkflowStageStatus(project, stage) === requestedStatus)?.id
...
export function groupProjectsByStage(
  projects: readonly WorkItem[],
  stages: readonly (WorkflowStage | string)[] = [...new Set(projects.flatMap(getProjectWorkflowStages).map((stage) => stage.id))],
): ProjectStageGroup[] {
  const availableStages = projects.flatMap(getProjectWorkflowStages);
  const knownStages = [...new Map(stages.flatMap((stage) => {
    if (typeof stage !== "string") return [[stage.id, stage] as const];
    const resolved = availableStages.find((candidate) => stageMatches(candidate, stage));
    return resolved ? [[resolved.id, resolved] as const] : [];
  }))].map(([, stage]) => stage);
  const unknownStages = projects
    .map(getProjectWorkflowStage)
    .filter((stage) => !knownStages.some((knownStage) => knownStage.id === stage.id));

  const allStages = [...new Map([...knownStages, ...unknownStages].map((stage) => [stage.id, stage] as const)).values()];
  return allStages.map((stage) => ({
    stage,
    projects: projects.filter((project) => getProjectWorkflowStage(project).id === stage.id),
  }));
}
...
export function getProjectStageMenuChoices(
  project: Pick<WorkItem, "status" | "title" | "workflowStageId" | "workflowStage" | "workflowStages">,
  stages: readonly (WorkflowStage | string)[] = getProjectWorkflowStages(project),
): ProjectStageMenuChoice[] {
  const availableStages = getProjectWorkflowStages(project);
  const choices = [...new Map(stages.flatMap((stage) => {
    const resolved = typeof stage === "string"
      ? availableStages.find((candidate) => stageMatches(candidate, stage))
      : stage;
    return resolved ? [[resolved.id, resolved] as const] : [];
  }))].map(([, stage]) => stage);
  const currentStage = getProjectWorkflowStage({ ...project, workflowStages: choices });
  return choices.map((stage) => {
    ...
    return {
      ...
      current,
      disabled: current,
      ariaLabel: current ? `${project.title}, current stage ${stage.label}` : `Move ${project.title} to ${stage.label}`,
    };
  });
}
...
export type ProjectDeliveryEffect =
  ...
  | { kind: "salary-plan"; completedProjects: number; requiredProjects: number; batchAmount: number };
...
export function getProjectDeliveryConfirmation(
  ...
  if (nextStatus !== "Delivered" || project.status === "Delivered") return { required: false };
  return {
    required: true,
...
export function moveProjectToStage(
  ...
  return { ...project, workflowStageId: workflowStage, workflowStage: undefined, ...projectStatusUpdate(project, status, changedAt) };
}
