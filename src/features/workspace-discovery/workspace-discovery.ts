...
export type SearchRecordKind =
  "client" | "project" | "group" | "output" | "file" | "action";
...
export function deriveCalendarEvents(
  ...
  for (const project of activeProjects) {
    ...
    const isClientPayment =
      ...
      project.workType !== (options.salaryWorkType ?? "Job / Salary");
    if (isClientPayment) {
      addEvent(events, {
  ...
  for (const output of outputs) {
    ...
    addEvent(events, {
      ...
      projectTitle: project.title,
      detail:
        output.reviewState === "changes_requested"
          ? "Changes requested"
          : "Review activity",
    });
  }
...
export function calendarFeedIcs(
...
export function buildWorkspaceSearchIndex(
  ...
  for (const client of input.clients ?? []) {
    ...
    records.push({
      ...
      kind: "client",
      title: client.name,
      detail: client.company || "Client",
      href: `/clients?client=${encodeURIComponent(client.id)}`,
      keywords: `${client.email} ${client.contactName}`,
  ...
  for (const group of input.groups ?? []) {
    ...
    records.push({
      ...
      href: `/projects?group=${encodeURIComponent(group.id)}`,
      keywords: group.notes,
  ...
  records.push(
    ...
    },
    {
      ...
      detail: "Action",
      href: "/files",
      keywords: "workspace files material",
    }
  );
  return records;
}
...
export function filterWorkspaceFiles(
  ...
  return files.filter(
    (file) =>
      !file.archived &&
      normalized(
        `${file.title} ${file.fileName} ${file.category} ${file.status}`
      ).includes(needle)
  );
}
