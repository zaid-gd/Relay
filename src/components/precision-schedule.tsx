...
function statusPalette(status: WorkItem["status"]) {
  if (status === "Delivered")
    return {
      fg: "var(--app-success)",
      bg: "color-mix(in srgb,var(--app-success)_14%,transparent)",
    };
  if (status === "In Progress")
    return {
      fg: "var(--app-warning)",
      bg: "color-mix(in srgb,var(--app-warning)_14%,transparent)",
    };
  if (status === "Cancelled")
    return {
      fg: "var(--app-danger)",
      bg: "color-mix(in srgb,var(--app-danger)_14%,transparent)",
    };
  if (
    status === "Review" ||
    status === "Revision" ||
...
export function PrecisionCalendar({
  ...
  const firstProjectDate = projects.find((project) =>
    parseDate(project.dueDate)
  )?.dueDate;
  const initialDate = firstProjectDate
    ? parseDate(firstProjectDate)!
    : todayDate();
  ...
  const events = useMemo(
    () =>
      deriveCalendarEvents(projects, outputs, {
        salaryWorkType: settings.salaryWorkType,
      }),
    [outputs, projects, settings.salaryWorkType]
  );

  const monthDays = useMemo(
    () => calendarMonthDays(visibleMonth, settings.weekStart),
    [visibleMonth, settings.weekStart]
  );
  const weekdays = useMemo(
    () => orderedWeekdays(settings.weekStart),
    [settings.weekStart]
  );
  ...
  const calendarViewRows = useMemo<
    Array<{ project: WorkItem | null; dateKey: string }>
  >(() => {
    if (viewMode === "agenda") {
      return visibleMonthProjects.map((project) => ({
        project,
        dateKey: project.dueDate,
      }));
    }
    if (viewMode === "week") {
      return selectedWeekDays.flatMap(
        (date): Array<{ project: WorkItem | null; dateKey: string }> => {
          const dateKey = iso(date);
          const matches = projects.filter(
            (project) => project.dueDate === dateKey
          );
  ...
  function downloadFeed() {
    const url = URL.createObjectURL(
      new Blob([calendarFeedIcs(events)], {
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            >
              <Download className="size-4" /> Subscribe .ics
            </Button>
            <Button
              variant="outline"
              className="h-10 border-[var(--app-highlight)] px-4 text-[var(--app-highlight)] hover:bg-[var(--app-active)]"
              onClick={jumpToToday}
            >
        ...
        <PageToolbar
            ...
            <>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className="size-9 border-[var(--app-border)] text-[var(--app-highlight)]"
              ...
              <div className="min-w-[170px]">
                ...
                <p className="text-[10px] text-[var(--app-muted)]">
                  {monthProjectCount} scheduled items
                </p>
              </div>
              <Tooltip>
                <TooltipTrigger asChild>
          ...
          secondary={
            <div className="flex flex-wrap items-center gap-2">
              <span className="hidden text-[10px] text-[var(--app-muted)] sm:inline">
                {monthProjectCount} scheduled
              </span>
              <Tabs
                value={viewMode}
                onValueChange={(value) =>
                  setViewMode(
                    value === "week" || value === "agenda" ? value : "month"
                  )
                }
                className="block"
              >
                <TabsList
                  ...
                  className="h-9 rounded-md border border-[var(--app-border)] bg-[var(--app-soft-panel)] p-0.5"
                >
            ...
            primary={
              viewMode === "month" ? (
                  ...
                  <>
                    <div className="grid grid-cols-7 border-l border-t border-[var(--app-border)]">
                      {weekdays.map((day) => (
                        <div
                          key={day}
                          className="border-b border-r border-[var(--app-border)] px-2 py-2 text-[11px] font-semibold uppercase text-[var(--app-muted)]"
                        >
                          {day}
                        </div>
                      ))}
                          ...
                          <motion.button
                            ...
                            className={cn(
                              ...
                              isSelected
                                ...
                                : isCurrentMonth
                                  ? "bg-[var(--app-panel)]"
                                  : "bg-[var(--app-soft-panel)] opacity-55",
                              isToday &&
                                !isSelected &&
                                "shadow-[inset_0_0_0_2px_var(--app-highlight)]"
                            )}
                          >
                            <span className="flex items-center justify-between gap-2">
                              ...
                              {dayEvents.length ? (
                                <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[var(--app-active)] px-1 text-[11px] font-semibold text-[var(--app-highlight)]">
                                  {dayEvents.length}
                                </span>
                              ) : null}
                            </span>
                            <span className="mt-2 block space-y-1">
                              {dayEvents.slice(0, 1).map((event) => {
                                ...
                                return (
                                  <span
                                    key={event.id}
                                    className="block truncate rounded border border-black/5 px-1.5 py-1 text-[11px] font-semibold"
                                    style={{
                ...
                <ContentSection
                  title={
                    viewMode === "week"
                      ? `Week of ${formatDate(iso(selectedWeekDays[0]), { month: "short", day: "numeric" })}`
                      : `${monthLabel} agenda`
                  }
                  description={
                    viewMode === "week"
                      ? "Seven-day delivery and review window."
                      : `${visibleMonthProjects.length} scheduled projects ordered by due date.`
                  }
                  bodyMode="flush"
                  className="h-full"
                >
                      ...
                      >
                        ...
                        <span className="min-w-0">
                          <span className="block truncate text-sm font-semibold">
                            {project?.title ?? "No scheduled delivery"}
                          </span>
                        ...
                        >
                          ...
                          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--app-progress-track)]">
                            <div
                              ...
                              style={{
                                width: `${projectProgress(project.status)}%`,
                              }}
                            />
                          </div>
                          <Button
...
export function PrecisionTimeline({
  ...
  const visibleProjects = useMemo(
    () =>
      projects.filter((project) => {
        if (filter === "Active")
          return !isDelivered(project) && project.status !== "Cancelled";
        if (filter === "Review")
          return ["Review", "Revision", "Client Review"].includes(
            project.status
          );
  ...
  const active = projects.filter(
    (project) => !isDelivered(project) && project.status !== "Cancelled"
  ).length;
  const review = projects.filter((project) =>
    ...
    <WorkspacePage family="data-index">
      <PageHeader
        ...
        actions={
          ...
          >
            <span className="rounded-md border border-[var(--app-border)] bg-[var(--app-soft-panel)] px-2.5 py-1.5">
              <strong className="text-base tabular-nums text-[var(--app-ink)]">
                {active}
              </strong>
              <span className="ml-1 text-[var(--app-muted)]">active</span>
            </span>
            <span className="rounded-md border border-[var(--app-border)] bg-[var(--app-soft-panel)] px-2.5 py-1.5">
              <strong className="text-base tabular-nums text-[var(--app-warning)]">
                {review}
              </strong>
              <span className="ml-1 text-[var(--app-muted)]">in review</span>
            </span>
            <span className="rounded-md border border-[var(--app-border)] bg-[var(--app-soft-panel)] px-2.5 py-1.5">
              <strong className="text-base tabular-nums text-[var(--app-success)]">
                {delivered}
              </strong>
              <span className="ml-1 text-[var(--app-muted)]">delivered</span>
            </span>
          </div>
        }
      />
        ...
        <PageToolbar
          ...
          secondary={
            <span
              ...
              aria-live="polite"
            >
              {visibleProjects.length} milestones shown
            </span>
          }
        />
        ...
        {groups.length ? (
            ...
            {groups.map((group, groupIndex) => (
              <motion.section
                ...
                transition={
                  reduceMotion
                    ? { duration: 0 }
                    : { ...revealTransition, delay: groupIndex * 0.035 }
                }
                  ...
                  >
                    <div className="divide-y divide-[var(--app-border)]">
                      {group.projects.map((project, projectIndex) => (
                        ...
                        >
                          <span
                            className="absolute -left-[31px] top-1/2 grid size-3 -translate-y-1/2 place-items-center rounded-full border-2 border-[var(--app-panel)]"
                            style={{
                              background: projectStatusColor(project.status),
                            }}
                          />
                          <span>
                            <span className="block text-[10px] font-semibold uppercase text-[var(--app-subtle)]">
                              {isDelivered(project) ? "Delivered" : "Expected"}
                            </span>
                          ...
                          <span className="flex items-center justify-end gap-1 text-[10px] text-[var(--app-muted)]">
                            {isDelivered(project) ? (
                              <CheckCircle2 className="size-3.5 text-[var(--app-success)]" />
                            ) : (
          ...
          <ContentSection className="mt-6">
            <PageEmptyState
              ...
              description="Projects with due dates will appear here."
            />
          </ContentSection>
        )}
      </PageContent>
