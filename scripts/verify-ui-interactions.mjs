...
async function withPage(
  ...
  run,
  {
    seedWorkspace = true,
    projectCount = 1,
    clientCount = Math.min(projectCount, 12),
  } = {}
) {
...
async function assertWorkspaceGeometry(page, route, expectedFamily) {
  const geometry = await page.evaluate(() => {
    ...
    return {
      ...
      main: rect(main),
      mainClientLeft: main
        ? Math.round(main.getBoundingClientRect().left + main.clientLeft)
        : 0,
  ...
  if (
    Math.abs(geometry.pageRoot.width - expectedWidth) > 1 ||
    Math.abs(geometry.pageRoot.left - expectedLeft) > 1
  ) {
  ...
  if (
    ...
    throw new Error(
      `${route} has inconsistent workspace gutters: ${leftGutter}px / ${rightGutter}px.`
    );
  }
  if (!["auto", "scroll"].includes(geometry.mainOverflowY)) {
    throw new Error(
      `${route} is not using the shell-owned primary scroll viewport.`
    );
  }
  if (geometry.mode === "fill" && !geometry.hasFillBody) {
    throw new Error(
      `${route} declares fill mode without a named fill viewport body.`
    );
  }
  if (
    geometry.mode === "document" &&
    ["auto", "scroll"].includes(geometry.rootOverflowY)
  ) {
    throw new Error(
      `${route} introduces a page-owned primary scroll container.`
    );
  }
}
...
async function assertApprovedFamilyDesigns(page) {
  ...
  const projectThumbnail = page
    .locator(
      '[data-slot="project-thumbnail"][data-thumbnail-kind="video"]:visible'
    )
    .first();
  try {
    ...
    const projectRows = await page
      .locator(
        '[data-testid="project-row"], [data-testid="mobile-project-row"]'
      )
      .count();
    throw new Error(
  ...
  if (process.env.CUTLAB_CAPTURE_FAMILIES === "1") {
    ...
    await page.screenshot({
      path: join(captureDirectory, "projects-restored.png"),
      fullPage: true,
    });
  }
  ...
  const calendarToolbar = page.locator(
    '[data-slot="page-toolbar"][data-family-toolbar="calendar"]'
  );
  ...
  const settingsNavigation = page.locator(
    '[data-slot="settings-navigation"][data-navigation-kind="icon-index"]'
  );
  if ((await settingsNavigation.count()) === 0) {
    throw new Error(
      "/settings is missing the approved icon-based settings index."
    );
  }
  await page
  ...
  await page
    .getByRole("heading", { level: 2, name: "Project Stages" })
    .waitFor();
  if (
    await page
      .getByRole("heading", { level: 2, name: "Workspace profile" })
    ...
    throw new Error(
      "/settings keeps every settings panel mounted instead of using the approved section workspace."
    );
  }
  ...
  for (const [route, selector, message] of [
    [
      ...
      "Feedback is missing its functional selected-review detail rail.",
    ],
    [
      "/resources",
      '[data-slot="page-toolbar"][data-family-toolbar="resources"]',
      "Resources is missing its library toolbar.",
    ],
    [
      "/templates",
      '[data-slot="template-card"]',
      "Templates is missing its visual template-card library.",
    ],
    [
      "/integrations",
      '[data-slot="page-toolbar"][data-family-toolbar="integrations"]',
      "Integrations is missing its grouped library toolbar.",
    ],
    [
      "/account",
      '[data-family-region="account-administration"]',
      "Account is missing its administration-family composition.",
    ],
  ]) {
...
async function assertInnerWorkspaceScroll(page, route, heading, scrollLabel) {
  ...
  if (!result.found || !("scrollTop" in result) || result.scrollTop <= 0) {
    throw new Error(
      `${route} does not provide a working inner scroll region named "${scrollLabel}": ${JSON.stringify(result)}`
    );
  }
  if (result.mainScrollTopAfter !== result.mainScrollTop) {
    throw new Error(
      `${route} moved the shell viewport while scrolling "${scrollLabel}".`
    );
  }
}
...
async function assertDashboardAndProjectInspectorRefinements(page) {
  ...
  const dashboardLayout = await page.evaluate(() => {
    ...
    return {
      ...
      metricColumnGap: metricStrip
        ? getComputedStyle(metricStrip).columnGap
        : "",
      ledger: rect(ledger),
  ...
  if (dashboardLayout.mainScrollbarWidth !== "none") {
    throw new Error(
      `Workspace viewport still exposes a native scrollbar: ${dashboardLayout.mainScrollbarWidth || "unknown"}.`
    );
  }
  if (dashboardLayout.metricColumnGap !== "0px") {
    throw new Error(
      `Dashboard KPI strip still has separator gaps: ${dashboardLayout.metricColumnGap || "unknown"}.`
    );
  }
  if (!dashboardLayout.ledger || !dashboardLayout.inspector) {
    throw new Error(
      "Dashboard project ledger or detail inspector could not be measured."
    );
  }
  if (
    Math.abs(dashboardLayout.ledger.top - dashboardLayout.inspector.top) > 1 ||
    Math.abs(dashboardLayout.ledger.bottom - dashboardLayout.inspector.bottom) >
      1
  ) {
    throw new Error(
      `Dashboard ledger and inspector are not level: ${JSON.stringify(dashboardLayout)}.`
    );
  }
  ...
  if (!projectInspector)
    throw new Error("Projects detail inspector could not be measured.");
  if (projectInspector.inspectorBottom > projectInspector.mainBottom + 1) {
    throw new Error(
      `Projects detail inspector is clipped by the workspace viewport: ${JSON.stringify(projectInspector)}.`
    );
  }
  if (
    ...
    throw new Error(
      `Projects detail inspector does not own working vertical scroll: ${JSON.stringify(projectInspector)}.`
    );
  }
}
...
try {
  await withPage(
    ...
    async (page) => {
      ...
      await page
        ...
        .waitFor();
      const initialProjectData = await page.evaluate(() =>
      ...
      await page
        ...
        .waitFor();
      await page
        .getByRole("heading", { name: "Good to see you, Maya." })
        .waitFor();
      await page.getByTestId("project-row").first().click();
      await page.getByRole("button", { name: /^Open project / }).click();
      await page.waitForURL(/\/sample-studio\/projects\/sample-/);
      await page
        .getByRole("navigation", { name: "Project workspace views" })
        .waitFor();
      const sampleProjectData = await page.evaluate(() =>
        localStorage.getItem("video-editing-work-tracker:v1")
      );
      if (sampleProjectData !== null)
        throw new Error(
          "The sample studio wrote project records to local storage."
        );
  ...
  await withPage(
    ...
    async (page) => {
      ...
      if (hasOverflow)
        throw new Error(
          "Mobile sample studio has document-level horizontal overflow."
        );
    },
    { seedWorkspace: false }
  );
  ...
  await withPage({ width: 1440, height: 1000 }, async (page) => {
    ...
    await page
      .getByPlaceholder("Search the project ledger")
      .fill("Interaction");
    await page.getByTestId("project-row").first().click();
  ...
  await withPage({ width: 1280, height: 900 }, async (page) => {
    ...
    if (!reduced)
      throw new Error("Reduced-motion media preference was not applied.");
    await page
      .locator('[data-slot="settings-navigation"]')
  ...
  await withPage({ width: 1280, height: 900 }, async (page) => {
    console.log(
      "Verifying every authenticated workspace route uses shared geometry..."
    );
    await page.goto(baseUrl, { waitUntil: "domcontentloaded" });
  ...
  await withPage(
    ...
    async (page) => {
      ...
      await assertInnerWorkspaceScroll(
        page,
        "/projects",
        "Projects",
        "Scrollable project library"
      );
      await assertInnerWorkspaceScroll(
        page,
        "/media",
        "Media",
        "Scrollable media packages"
      );
      await assertInnerWorkspaceScroll(
        page,
        "/clients",
        "Clients",
        "Scrollable client project history"
      );
      console.log(
        "Verifying shell, Dashboard alignment, KPI strip, and Projects inspector refinements..."
      );
  ...
  for (const viewport of [
    ...
    await withPage(viewport, async (page) => {
      console.log(
        `Verifying representative layout families at ${viewport.width}px...`
      );
      await page.goto(baseUrl, { waitUntil: "domcontentloaded" });
  ...
  console.log(
    "UI interactions and shared workspace geometry verified across all authenticated routes and responsive acceptance widths."
  );
} finally {
  await browser.close();
...
async function waitForServer(url, getOutput) {
  ...
  while (Date.now() - started < startupTimeoutMs) {
    ...
    if (server?.exitCode !== null) {
      throw new Error(
        `Production server exited before UI verification.\n${getOutput()}`
      );
    }
    await delay(300);
  }
