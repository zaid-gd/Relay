...
function contrastRatio(foreground, background) {
  ...
  const foregroundLuminance =
    0.2126 * channel(foregroundRgb[0]) +
    0.7152 * channel(foregroundRgb[1]) +
    0.0722 * channel(foregroundRgb[2]);
  const backgroundLuminance =
    0.2126 * channel(backgroundRgb[0]) +
    0.7152 * channel(backgroundRgb[1]) +
    0.0722 * channel(backgroundRgb[2]);
...
const result = await page.evaluate(() => {
  ...
  sample.style.color = "var(--app-highlight)";
  sample.style.backgroundColor = "var(--app-panel)";
...
if (result.theme !== "light")
  throw new Error("Expected light theme, received " + result.theme + ".");
const ratio = contrastRatio(result.highlight, result.panel);
if (!Number.isFinite(ratio) || ratio < 4.5) {
  throw new Error(
    "Light-mode highlight contrast is " +
      ratio.toFixed(2) +
      ":1 (" +
      result.highlight +
      " on " +
...
console.log(
  "Light-mode highlight contrast verified at " + ratio.toFixed(2) + ":1."
);
await browser.close();
