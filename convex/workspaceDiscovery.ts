...
async function visibleProjects(ctx: QueryCtx, userId: string) {
  const personal = await ctx.db
    .query("projects")
    .withIndex("by_ownerUserId_and_teamId", (q) => q.eq("ownerUserId", userId).eq("teamId", undefined))
    .take(MAX_PROJECTS);
  ...
  const visibleTeamProjects = membership.role === "Owner" || workspace?.allowAllTeamProjects
    ? teamProjects
    : teamProjects.filter((project) => project.ownerUserId === userId || project.assigneeUserIds.includes(userId));
...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    const teamOwnerId = membership
      ...
      : undefined;
    const settings = teamOwnerId ? await workspaceSettings(ctx, teamOwnerId) : personalSettings;
    const includeArchived = args.includeArchived === true;
    const visible = includeArchived ? projects : projects.filter((project) => !project.archived);
    const clientRecords = (settings?.clients ?? []).filter((client) => includeArchived || !client.archived);
    ...
    for (const project of visible) {
      ...
      for (const output of projectOutputs) {
        ...
        outputs.push({ id: output.id, projectId: output.projectId, title: output.title, dueDate: output.dueDate, reviewState: output.reviewState, archived: output.archived, updatedAt: output.updatedAt });
      }
      ...
      for (const file of projectFiles) {
        ...
        files.push({
          ...
          projectOutputId: file.projectOutputId ? String(file.projectOutputId) : undefined,
          title: file.title,
          ...
          url: version?.storageId ? (await ctx.storage.getUrl(version.storageId)) ?? undefined : version?.externalUrl ?? undefined,
        });
      }
    }
    return {
      clients: clientRecords.map((client) => ({ id: client.id, name: client.name, company: client.company, archived: client.archived })),
      groups: visibleGroups.map((group) => ({ id: group.id, name: group.name, clientId: group.clientId, archived: group.archived })),
      projects: visible.map((project): VisibleProject & { clientName: string } => ({
