...
async function requireProjectAccess(ctx: FunctionCtx, projectId: string, permission: ProjectPermission) {
...
async function sharedVersion(
  ...
  const output = await ctx.db
    ...
    .unique();
  if (!output || output.projectId !== portal.projectId || output.archived) return null;
  const version = await ctx.db
    .query("projectMediaVersions")
    .withIndex("by_versionId", (q) => q.eq("id", mediaVersionId))
    .unique();
  if (!version || version.outputId !== output._id || output.currentMediaVersionId !== version._id) return null;
  return { output, version };
}

function publicComment(comment: Doc<"mediaVersionComments">, outputId: string, mediaVersionId: string) {
  return {
    id: comment._id,
    outputId,
    mediaVersionId,
...
async function currentComments(ctx: QueryCtx, portal: Doc<"projectPortals">) {
  ...
  for (const outputId of portal.selectedOutputIds.slice(0, 20)) {
    const output = await ctx.db
      ...
      .withIndex("by_outputId", (q) => q.eq("id", outputId))
      .unique();
    if (!output || output.projectId !== portal.projectId || output.archived || !output.currentMediaVersionId) continue;
    const versions = await ctx.db
      .query("projectMediaVersions")
      .withIndex("by_outputId_and_versionNumber", (q) => q.eq("outputId", output._id))
      .take(100);
    const current = versions.find((version) => version._id === output.currentMediaVersionId);
    if (!current) continue;
    const rows = await ctx.db
      .query("mediaVersionComments")
      .withIndex("by_mediaVersionId", (q) => q.eq("mediaVersionId", current._id))
      .order("desc")
      .take(MAX_COMMENTS);
    comments.push(...rows.map((comment) => publicComment(comment, output.id, current.id)));
  }
  return comments.sort((left, right) => right.createdAt.localeCompare(left.createdAt)).slice(0, MAX_COMMENTS);
}
...
export const listForPortal = query({
  ...
  handler: async (ctx, args) => {
    ...
    return { access: "active" as const, comments: await currentComments(ctx, result.portal) };
  },
});
...
export const addPublicComment = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const shared = await sharedVersion(ctx, result.portal, args.outputId, args.mediaVersionId);
    if (!shared) throw new Error("Comments can only be added to the current shared Media Version");
...
export const reopenPublicComment = mutation({
  args: { token: v.string(), pin: v.optional(v.string()), commentId: v.id("mediaVersionComments") },
  handler: async (ctx, args) => {
    const result = await getPublicPortalAccess(ctx, args.token, args.pin);
    if (!result.portal) throw new Error(`Portal access ${result.access}`);
    const comment = await ctx.db.get("mediaVersionComments", args.commentId);
    if (!comment || comment.projectId !== result.portal.projectId) throw new Error("Comment not found");
    const output = await ctx.db.get("projectOutputs", comment.outputId);
    if (!output) throw new Error("Comment Output not found");
    const version = await ctx.db.get("projectMediaVersions", comment.mediaVersionId);
    if (!version) throw new Error("Comment Media Version not found");
    const shared = await sharedVersion(ctx, result.portal, output.id, version.id);
    if (!shared) throw new Error("Only comments on the current shared Media Version can be reopened");
...
export const listForProject = query({
  ...
  handler: async (ctx, args) => {
    ...
    const outputs = await ctx.db
      ...
      .take(100);
    const outputById = new Map(outputs.map((output) => [output._id, output.id]));
    const comments: Array<ReturnType<typeof publicComment>> = [];
    for (const output of outputs) {
      const versions = await ctx.db
        ...
        .withIndex("by_outputId_and_versionNumber", (q) => q.eq("outputId", output._id))
        .take(100);
      const versionById = new Map(versions.map((version) => [version._id, version.id]));
      const rows = await ctx.db
        .query("mediaVersionComments")
        .withIndex("by_outputId_and_resolved", (q) => q.eq("outputId", output._id))
        .order("desc")
        .take(MAX_COMMENTS);
      comments.push(...rows.map((comment) => publicComment(
        comment,
...
export const setResolved = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return publicComment(updated, output?.id ?? updated.outputId, version?.id ?? updated.mediaVersionId);
  },
});
