...
import { requireProjectAccess, canSeeTeamProject } from "./projectAccess";
...
async function requirePaymentAccess(
  ...
  if (
    !membership ||
    !(membership.permissions.manageFinance ?? membership.role === "Owner")
  )
    throw new Error("Permission denied");
  return identity;
}

async function isLegacySalaryProject(
  ctx: MutationCtx,
  project: Doc<"projects">
) {
  ...
  const ownerUserId = project.teamId
    ? workspaceId
      ? (await ctx.db.get(workspaceId))?.ownerUserId
      : undefined
    : project.ownerUserId;
  ...
  return (
    ...
    (settings?.salaryWorkType ?? "Job / Salary").trim().toLowerCase()
  );
}
...
async function validatedAssignees(
  ctx: FunctionCtx,
  ...
  const memberships = await Promise.all(
    unique.map((userId) => activeMembership(ctx, teamId, userId))
  );
  if (memberships.some((membership) => !membership))
    throw new Error("Project assignees must be active Workspace members");
  return unique;
}
...
function normalizeWorkflow(stages: WorkflowStage[]) {
  if (stages.length < 2 || stages.length > 12)
    throw new Error("Workflow needs 2 to 12 stages");
  ...
  if (
    !normalized.length ||
    normalized.some((stage) => !stage.id || !stage.label)
  ) {
...
async function settingsOwnerId(
  ctx: FunctionCtx,
  teamId: string | undefined,
  userId: string
) {
...
async function validateClientAndGroup(
  ...
  const settings = await ctx.db
    ...
    .unique();
  const client = await getWorkspaceClient(ctx, ownerId, args.clientId);
  if (!settings || !client || client.archived) {
  ...
  const group = teamId
    ? await ctx.db
        ...
        .withIndex("by_teamId_and_id", (q) =>
          q.eq("teamId", teamId).eq("id", projectGroupId)
        )
        .unique()
    : await ctx.db
        .query("projectGroups")
        .withIndex("by_userId_and_id", (q) =>
          q.eq("userId", args.userId).eq("id", projectGroupId)
        )
        .unique();
  if (!group || group.archived || group.clientId !== args.clientId) {
    throw new Error(
      "Project Group must be active and belong to the selected Client"
    );
  }
  return settings;
}

async function getSalaryPlan(
  ctx: FunctionCtx,
...
async function validateSalaryPlan(
  ...
  if (teamId)
    throw new Error("Salary Plans are available for solo Projects only");
  ...
  if (plan.clientId !== clientId)
    throw new Error("Project Client must match the Salary Plan");
  return plan;
}
...
async function deliveryEffect(
  ...
  if (
    !plan &&
    project.workType !== (settings?.salaryWorkType ?? "Job / Salary")
  ) {
    return {
  ...
  const requiredProjectCount =
    ...
    Math.max(1, Math.floor(settings?.salaryBatchSize ?? 20));
  const amount =
    plan?.amount ?? Math.max(0, settings?.salaryBatchAmount ?? 10000);
  ...
  const settledProjectIds = new Set(
    batches
      .filter((batch) =>
        plan ? batch.salaryPlanId === plan._id : !batch.salaryPlanId
      )
  ...
  return {
    ...
    plan,
    clientName: plan
      ? (await getWorkspaceClient(ctx, ownerUserId, plan.clientId))?.name
      : undefined,
  };
}
...
export const list = query({
  ...
  handler: async (ctx) => {
    ...
    const personal = await ctx.db
      ...
      .take(501);
    if (personal.length > 500)
      throw new Error("Use projects.listPage to load more than 500 projects");
    const membership = await ctx.db
      .query("teamMembers")
    ...
    const teamProjects = await ctx.db
      ...
      .take(501);
    if (teamProjects.length > 500)
      throw new Error("Use projects.listPage to load more than 500 projects");
    const visibleTeamProjects = teamProjects.filter((project) =>
      canSeeTeamProject(
        project,
        membership,
...
export const listSalaryBatches = query({
  args: {},
  returns: v.array(schema.doc("projectSalaryBatches")),
  handler: async (ctx) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) return [];
    const batches = await ctx.db
    ...
    if (batches.length > 500)
      throw new Error(
        "Use projects.salaryBatchesPage to load more than 500 batches"
      );
    return batches;
  },
});
...
export const listPage = query({
  ...
  returns: v.object({
    page: v.array(schema.doc("projects")),
    isDone: v.boolean(),
  ...
  handler: async (ctx, { scope, paginationOpts }) => {
    ...
    const membership = await ctx.db
      ...
      .first();
    if (!membership?.permissions.viewProjects)
    ...
    const result = await ctx.db
      ...
      .paginate(paginationOpts);
    return {
      page: result.page.filter((project) =>
        canSeeTeamProject(
          project,
          membership,
          workspace?.allowAllTeamProjects ?? false
...
export const salaryBatchesPage = query({
  args: { paginationOpts: paginationOptsValidator },
  returns: v.object({
    page: v.array(schema.doc("projectSalaryBatches")),
    isDone: v.boolean(),
    continueCursor: v.string(),
  }),
  ...
  handler: async (ctx, args) => {
    ...
    const batch = await ctx.db
      ...
      .withIndex("by_ownerUserId_and_id", (q) =>
        q.eq("ownerUserId", identity.tokenIdentifier).eq("id", args.batchId)
      )
      .unique();
    if (!batch) throw new Error("Salary Batch not found");
    const receivedAt = args.paid
...
export const importSalaryBatches = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      ...
      .first();
    if (existing)
      throw new Error("Salary Batch import requires an empty Workspace");
    const seenProjectIds = new Set<string>();
    const seenBatchIds = new Set<string>();
    for (const batch of args.batches) {
      if (
        !batch.id.trim() ||
        seenBatchIds.has(batch.id) ||
        !Number.isInteger(batch.requiredProjectCount) ||
        batch.requiredProjectCount < 1 ||
        !Number.isFinite(batch.amount) ||
        batch.amount < 0 ||
      ...
      const projects = await Promise.all(
        batch.projectIds.map((id) => getProject(ctx, id))
      );
      if (
        projects.some(
          (project) =>
            ...
            project.teamId ||
            project.workType !== batch.workType ||
...
export const create = mutation({
  ...
  handler: async (ctx, { project }) => {
    ...
    const existing = await ctx.db
      ...
      .unique();
    if (
      existing?.ownerUserId === identity.tokenIdentifier &&
    ...
    if (reservedByBatch)
      throw new Error("Project id is reserved by Salary Batch history");
    ...
    const assigneeUserIds = await validatedAssignees(
      ctx,
      project.teamId,
      project.assigneeUserIds
    );
    const { starterOutputs = [], ...projectFields } = project;
    if (starterOutputs.length > 20)
      throw new Error("A Template can create at most 20 Project Outputs");
    await ctx.db.insert("projects", {
      ...
      earnings:
        salaryPlan ||
        (!project.teamId && project.workType === settings.salaryWorkType)
          ? 0
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const projectGroupId =
      args.changes.projectGroupId === null
        ? undefined
        : (args.changes.projectGroupId ?? project.projectGroupId);
    const salaryPlanId =
      args.changes.salaryPlanId === null
        ? undefined
    ...
    if (salaryPlanId !== project.salaryPlanId) {
      const settledBatch = (
        await ctx.db
          .query("projectSalaryBatches")
          .withIndex("by_ownerUserId", (q) =>
            q.eq("ownerUserId", project.ownerUserId)
          )
          .collect()
      ).some((batch) => batch.projectIds.includes(project.id));
      if (settledBatch)
        throw new Error(
          "A Project in a completed Salary Batch cannot change Salary Plans"
        );
    }
    const settings = await validateClientAndGroup(ctx, {
      userId: identity.tokenIdentifier,
    ...
    if (args.changes.title !== undefined && !title)
      throw new Error("Project title is required");
    const workType = args.changes.workType ?? project.workType;
    const earnings =
      salaryPlan || (!project.teamId && workType === settings.salaryWorkType)
        ? 0
    ...
    const assigneeUserIds =
      args.changes.assigneeUserIds === undefined
        ? undefined
        : await validatedAssignees(
            ctx,
            project.teamId,
            args.changes.assigneeUserIds
          );
    await ctx.db.patch(project._id, {
      ...
      ...(args.changes.notes === undefined
        ? {}
        : { notes: args.changes.notes.trim().slice(0, 4000) }),
...
export const setPayment = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (project.status !== "Delivered")
      throw new Error("Only delivered Projects can be marked paid");
    if (project.salaryPlanId)
      throw new Error("Salary Plan Projects use Salary Batch payment tracking");
    ...
    const paidDate = args.paid
      ? (project.paidDate ?? new Date().toISOString())
      : undefined;
    await ctx.db.patch(project._id, {
      paid: args.paid,
      paidDate,
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (project.teamId) {
      ...
      const canRemove =
        membership?.permissions.manageTeam ||
        (project.ownerUserId === identity.tokenIdentifier &&
...
export const previewStage = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!stage)
      throw new Error("Workflow stage does not belong to this Project");
    if (stage.purpose !== "delivered") return { kind: "none" as const };
    if (project.teamId)
      return { kind: "client" as const, earned: project.earnings };
    return (await deliveryEffect(ctx, identity.tokenIdentifier, project))
      .result;
  },
});
...
export const transitionStage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (effect.result.batchCreated) {
      const number =
        batches.reduce((highest, batch) => Math.max(highest, batch.number), 0) +
        1;
      await ctx.db.insert("projectSalaryBatches", {
        ownerUserId: identity.tokenIdentifier,
