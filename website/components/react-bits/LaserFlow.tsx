...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const applyBackgroundMode = (value: string) => {
      ...
      const background = new THREE.Color(
        isTransparent ? "#000000" : value || "#000000"
      );
      const luma =
        background.r * 0.2126 + background.g * 0.7152 + background.b * 0.0722;
      const lightBackground = luma > 0.72;
      mount.style.backgroundColor = isTransparent
        ? "transparent"
        : value || "#000000";
      canvas.style.filter = lightBackground
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      const background = new THREE.Color(
        isTransparent ? "#000000" : backgroundColor || "#000000"
      );
      const luma =
        background.r * 0.2126 + background.g * 0.7152 + background.b * 0.0722;
      const lightBackground = luma > 0.72;
